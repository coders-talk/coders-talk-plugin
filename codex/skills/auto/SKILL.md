...
1. Run this command exactly, with `<mode>` replaced by what the user wrote after the command name (`on`, `team`, `push` or `off`), or with nothing in its place if they wrote nothing:
...
- If the user wrote something other than `on`, `team`, `push` or `off`, run the command without a mode and show them the result.
