---
name: share
description: Same as $coders-talk:build. Send this Codex session to Coders Talk as a draft Build that the user reviews and publishes on the site. Use only when the user asks for $coders-talk:share.
---
...
In the commands below, `<plugin>` is the absolute path of the plugin folder: this file is `<plugin>/codex/skills/share/SKILL.md`. Run every command outside the sandbox (with escalated permissions): the script reads the Codex session and talks to coders.talk. Give "Send this session to Coders Talk" as the reason; the user approves.
