#!/usr/bin/env node
/**
 * SessionEnd hook: auto mode's send of the session that ended (lib/hooks.mjs).
...
await runHook(process.argv.includes('--agent=codex') ? 'codex' : 'claude-code', 'session-end', { snapshot: false });
