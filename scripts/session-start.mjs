#!/usr/bin/env node
/**
 * SessionStart hook: HEAD at the start of the session, and auto mode's catching up (lib/hooks.mjs).
...
await runHook(process.argv.includes('--agent=codex') ? 'codex' : 'claude-code', 'session-start', { snapshot: false });
